...
class CodeCreater:
    ...
    def injection_code():
        ...
        time.sleep(0.5)
        interface = input(colr().hex("#fff300", "\n > ", rgb_mode=True))
        time.sleep(0.5)
        ...
        def show_execute(name_upper, name_lower):
            Colors.light_blue(
                "\n [1] SHOW THE COMMAND  [2] Execute   \n [3] RF-KILL_ERROR     [4] Exit"
            )
            px = input(colr().hex("#fff300", "\n > ", rgb_mode=True))
            ...
            def show_the_command(name_upper, name_lower):
                ...
                time.sleep(0.5)
                Colors.light_blue("\n CHANGE MODE TO AUTO")
                Colors.green(f"\n $ sudo iwconfig {name_lower} mode Managed")
                time.sleep(0.5)
                Colors.light_blue(f"\n {name_upper} UP COMMAND")
                time.sleep(0.5)
                Colors.green(f"\n $ sudo ifconfig {name_lower} up")
            ...
            def execute_the_command(name_upper, name_lower):
                ...
                def mmca_rnm():
                    time.sleep(0.5)
                    Colors.light_blue(f"\n Monitor Mode Disable From {name_upper}  ")
                    time.sleep(0.5)
                    Colors.red("")
                    os.system(
                        f"sudo ifconfig {name_lower} down && sudo iwconfig {name_lower} mode Managed && sudo ifconfig {name_lower} up"
                    )
                    time.sleep(0.5)
                    Colors.orange(" Network Manager Restarted")
                    time.sleep(0.5)
                    Colors.red("")
                    os.system(f"sudo systemctl restart NetworkManager")
                    time.sleep(0.5)
                    Colors.light_blue(
                        "\n SHOWING THE IWCONFIG RESULT. \n CHECK MONITOR MODE IS DISABLE"
                    )
                    time.sleep(0.5)
                    Colors.red("")
                    os.system(f"iwconfig {name_lower}")
                    Operators.exit_author()
            ...
            # RF-KILL Error meassage solution function
            def rf_kill_error():
                null = ""
                dash = null.center(81, "-")
                ...
                print(
                    ...
                    colr().hex(
                        "#ff0000",
                        "Error [1] : ioctl(SIOCSIFFLAGS) failed: Operation not possible due to RF-kill",
                        rgb_mode=True,
                    ),
                    colr().hex("#6666ff", "|", rgb_mode=True),
                )
                print(
                    ...
                    colr().hex(
                        "#ff0000",
                        "Error [2] : SIOCSIFFLAGS: Operation not possible due to RF-kill",
                        rgb_mode=True,
                    ),
                    colr().hex("#6666ff", "              |", rgb_mode=True),
                )
                Colors.light_blue("     " + dash)
                Colors.orange(
                    " \n If you get this like error in the step of injection time or stop monitor mode y or n "
                )
                choice = input(colr().hex("#fff300", "\n > ", rgb_mode=True))
                choice = choice.lower()

                # RF-KILL  solution function
                def rf_kill_solving():
                    Colors.light_blue("\n [+] Copying Bash file to /usr/local/bin/")
                    Colors.red("")
                    os.system(
                        "sudo cp -v unblock_wifi.sh /usr/local/bin/ && chmod +x /usr/local/bin/unblock_wifi.sh"
                    )
                    Colors.light_blue(
                        "\n [+] Copying Service file to /etc/systemd/system/"
                    )
                    Colors.red("")
                    os.system("sudo cp -v unblock-wifi.service /etc/systemd/system/")
                    Colors.light_blue("\n [+] Enable and Start the Service")
                    Colors.red("")
                    os.system(
                        "sudo systemctl enable unblock-wifi.service && sudo systemctl start unblock-wifi.service"
                    )
                    Colors.orange(
                        " \n If you continue with the reboot , (you must reboot your system) y or n "
                    )
                    choice = input(colr().hex("#fff300", "\n > ", rgb_mode=True))
                    choice = choice.lower()
                    try:
                        if choice == "y" or choice == "yes":
                            ...
                            Colors.red(
                                "\n You must reboot your system.\n After the reboot your service will start working"
                            )
                            choice = input(
                                colr().hex("#fff300", "\n > ", rgb_mode=True)
                            )
